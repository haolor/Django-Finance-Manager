...
def _load_env_file(env_path: Path) -> None:
    """
    Load key=value pairs from a local .env/.env.local file into process env.
    (Development helper; avoids adding extra dependencies.)
    """
    try:
        ...
        # Don't block app boot due to env parsing errors.
        return
...
ALLOWED_HOSTS = [
    h.strip()
    for h in os.environ.get('DJANGO_ALLOWED_HOSTS', 'localhost,127.0.0.1,core-api').split(',')
    if h.strip()
]
...
DATABASES = {
    'default': {
        ...
        'PASSWORD': os.environ.get('POSTGRES_PASSWORD', 'Kien04112004!'),
...
# ----------------------------
# Read credentials from environment variables (prefer .env.local).
